...
```powershell
docker save -o cidstore-py314-slim.tar cidstore:py314-slim
# Share cidstore-py314-slim.tar with cidsem team
```

**Import image (cidsem team runs this):**
```powershell
docker load -i cidstore-py314-slim.tar
docker tag cidstore:py314-slim cidstore:latest
docker run -d --name cidstore -p 8000:8000 -p 5555:5555 -p 5556:5556 -p 5557:5557 cidstore:latest
```

## Notes

- **Python version**: Uses official `python:3.14-slim` base (no custom compile needed)
- **Free-threading**: This image uses standard Python 3.14. If you need GIL-free execution, use a custom-built image (see `docs/Dockerfile.custom-py314`)
